...
export function ClubMap({ latitude, longitude, clubName, className = "" }: ClubMapProps) {
  // Generate a stable key based on coordinates to ensure proper re-initialization
  // when coordinates change, preventing "MapContainer is already initialized" error
  ...
  return (
    <div
      ...
      className={`rsp-club-map-wrapper ${className}`.trim()}
    >
      <MapContainer
        key={mapKey}
